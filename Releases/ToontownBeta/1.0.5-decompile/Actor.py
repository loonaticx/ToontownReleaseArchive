...
class Actor(PandaObject, NodePath):
    ...
    def __init__(self, models = None, anims = None, other = None):
        ...
        self._Actor__geomNode.arc().setFinal(1)
    ...
    def cleanup(self):
        self.stop()
        self._Actor__partBundleDict = None
        self._Actor__animControlDict = None
    ...
    def getLODNames(self):
        return self._Actor__partBundleDict.keys()
    ...
    def update(self, lod = 0):
        if lod < len(self._Actor__partBundleDict.values()):
            partBundles = self._Actor__partBundleDict.values()[lod].values()
    ...
    def getFrameRate(self, animName = None, partName = None):
        ...
        if partName == None:
            partName = self._Actor__animControlDict[lodName].keys()[0]
        
        if animName == None:
            animName = self.getCurrentAnim(partName)
        
        if self._Actor__animControlDict[lodName].has_key(partName):
    ...
    def setPlayRate(self, rate, animName = None, partName = None):
        for lodName in self._Actor__animControlDict.keys():
            animControlDict = self._Actor__animControlDict[lodName]
            if partName == None:
    ...
    def getCurrentAnim(self, partName = None):
        ...
        if self._Actor__animControlDict[lodName].has_key(partName):
            for animName in self._Actor__animControlDict[lodName][partName].keys():
    ...
    def getPart(self, partName, lodName = 'lodRoot'):
        if self._Actor__partBundleDict.has_key(lodName):
    ...
    def removePart(self, partName, lodName = 'lodRoot'):
        if self._Actor__partBundleDict.has_key(lodName):
            ...
            Actor.notify.warning('no lod named: %s' % lodName)
            return None
        if self._Actor__animControlDict.has_key(lodName):
            animControlDict = self._Actor__animControlDict[lodName]
        else:
            Actor.notify.warning('no lod named: %s' % lodName)
            return None
    ...
    def stop(self, animName = None, partName = None):
        for thisLod in self._Actor__animControlDict.keys():
            animControlDict = self._Actor__animControlDict[thisLod]
            if partName == None:
                partNames = animControlDict.keys()
            else:
                partNames = [
            ...
            for thisPart in partNames:
                for thisAnim in animNames:
                    if isinstance(animControlDict[thisPart][thisAnim][1], AnimControl):
        ...
        for thisLod in self._Actor__animControlDict.keys():
            ...
            if partName == None:
                for thisPart in animControlDict.keys():
                    animControl = self.getAnimControl(animName, thisPart, thisLod)
                    if animControl != None:
                        if fromFrame == None:
                            animControl.play()
                        else:
                ...
                animControl = self.getAnimControl(animName, partName, thisLod)
                if animControl != None:
                    if fromFrame == None:
                        animControl.play()
                    else:
    ...
    def loadModel(self, modelPath, partName = 'modelRoot', lodName = 'lodRoot', copy = 1):
        ...
        if bundle.isEmpty():
            ...
            if self._Actor__partBundleDict.has_key(lodName) == 0:
                ...
                needsDict = 0
            if lodName != 'lodRoot':
                bundle = bundle.instanceTo(self._Actor__LODNode.find('**/' + str(lodName)))
            else:
                bundle = bundle.instanceTo(self._Actor__geomNode)
            if needsDict:
    ...
    def loadAnims(self, anims, partName = 'modelRoot', lodName = 'lodRoot'):
        ...
        for animName in anims.keys():
            if not self._Actor__animControlDict.has_key(lodName):
                lodDict = { }
                self._Actor__animControlDict[lodName] = lodDict
            
            if not self._Actor__animControlDict[lodName].has_key(partName):
                partDict = { }
                self._Actor__animControlDict[lodName][partName] = partDict
            
            if not len(self._Actor__animControlDict[lodName][partName].keys()):
                animDict = { }
                self._Actor__animControlDict[lodName][partName] = animDict
    ...
    def __bindAnimToPart(self, animName, partName, lodName):
        ...
        anim.removeNode()
        bundleNode = self._Actor__partBundleDict[lodName][partName].node()
