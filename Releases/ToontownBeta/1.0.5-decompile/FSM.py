...
class FSM(DirectObject):
    ...
    def __init__(self, name, states = [], initialStateName = None, finalStateName = None):
        self.setName(name)
    ...
    def enterInitialState(self):
        self._FSM__enter(self._FSM__initialState)
        return None
    ...
    def request(self, aStateName, enterArgList = [], exitArgList = []):
        ...
        if aStateName in self._FSM__currentState.getTransitions():
            ...
            FSM.notify.warning('[%s]: no transition exists from %s to %s' % (self._FSM__name, self._FSM__currentState.getName(), aStateName))
            return 0
