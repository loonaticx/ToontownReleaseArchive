...
def __getSoundTrack(level, delay):
    ...
    featherTrack = MultiTrack([
        ...
        Track([
            ...
            FunctionInterval(scaleFeathers, extraArgs = [
                feathers]),
            MovieUtil.getScaleIntervals(feathers, dFeatherScaleUp, MovieUtil.ZERO_SCALE, feathers[0].getScale)]),
        Track([
            WaitInterval(toon.getDuration('tickle') - dFeatherScaleDown),
            MovieUtil.getScaleIntervals(feathers, dFeatherScaleDown, None, MovieUtil.ZERO_SCALE)])])
    tHeal = 3.0
    mtrack = MultiTrack([
        featherTrack,
        Track([
            ActorInterval(toon, 'tickle')]),
        __getSoundTrack(level, 1),
        Track([
...
def __healJoke(heal):
    ...
    tracks.append(Track([
        ...
        FunctionInterval(MovieUtil.showProps, extraArgs = [
            megaphones,
            hands]),
        MovieUtil.getScaleIntervals(megaphones, dMegaphoneScale, MovieUtil.ZERO_SCALE, Point3(1, 1, 1)),
        WaitInterval(toon.getDuration('sound') - 2.0 * dMegaphoneScale),
        MovieUtil.getScaleIntervals(megaphones, dMegaphoneScale, Point3(1, 1, 1), MovieUtil.ZERO_SCALE),
        FunctionInterval(MovieUtil.removeProps, extraArgs = [
            megaphones])]))
    tracks.append(Track([
        WaitInterval(tDoSoundAnimation),
        ActorInterval(toon, 'sound')]))
    soundTrack = __getSoundTrack(level, 2.0)
...
def __healSmooch(heal):
    ...
    lipstickIvals = [
        ...
        MovieUtil.getScaleIntervals(lipsticks, dScale, MovieUtil.ZERO_SCALE, Point3(1, 1, 1)),
        WaitInterval(toon.getDuration('smooch') - 2.0 * dScale),
        MovieUtil.getScaleIntervals(lipsticks, dScale, Point3(1, 1, 1), MovieUtil.ZERO_SCALE),
        FunctionInterval(MovieUtil.removeProps, extraArgs = [
            lipsticks])]
    ...
    lipsIvals = [
        ...
        FunctionInterval(MovieUtil.showProp, extraArgs = [
            ...
            render,
            getLipPos]),
        FunctionInterval(lips.setBillboardPointWorld),
        LerpScaleInterval(lips, dScale, Point3(3, 3, 3), startScale = MovieUtil.ZERO_SCALE),
...
def __healDance(heal):
    ...
    propIvals = [
        ...
        MovieUtil.getScaleIntervals(hats + canes, dScale, MovieUtil.ZERO_SCALE, Point3(1, 1, 1)),
        WaitInterval(toon.getDuration('happy-dance') - 2.0 * dScale),
        MovieUtil.getScaleIntervals(hats + canes, dScale, Point3(1, 1, 1), MovieUtil.ZERO_SCALE),
        FunctionInterval(MovieUtil.removeProps, extraArgs = [
            hats]),
        FunctionInterval(MovieUtil.removeProps, extraArgs = [
            canes])]
...
def __healSprinkle(heal):
    ...
    mtrack = MultiTrack([
        __getPartTrack(sprayEffect, 1.5, 0.5, [
            ...
            toon,
            0]),
        __getPartTrack(dropEffect, 1.9, 2.0, [
            dropEffect,
            target,
            0]),
        __getPartTrack(explodeEffect, 2.7, 1.0, [
            explodeEffect,
            toon,
            0]),
        __getPartTrack(poofEffect, 3.4, 1.0, [
            poofEffect,
            target,
            0]),
        __getPartTrack(wallEffect, 4.05, 1.2, [
            wallEffect,
            toon,
            0]),
        __getSoundTrack(level, 2),
