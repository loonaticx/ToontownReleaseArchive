...
# File: TownBattleToonPanel.pyo (Python 2.0)
...
class TownBattleToonPanel(DirectFrame):
    ...
    def __init__(self, id):
        gui = loader.loadModelOnce('phase_5/models/gui/battle_gui')
        DirectFrame.__init__(self, relief = None, image = gui.find('**/ToonBtl_Status_BG'), image_color = Vec4(0.5, 0.9, 0.5, 1))
        self.initialiseoptions(TownBattleToonPanel)
        self.avatar = None
        self.sosText = DirectLabel(parent = self, relief = None, pos = (0.1, 0, 0.015), text = 'SOS', text_scale = 0.06)
        self.sosText.hide()
        self.undecidedText = DirectLabel(parent = self, relief = None, pos = (0.1, 0, 0.015), text = '?', text_scale = 0.1)
