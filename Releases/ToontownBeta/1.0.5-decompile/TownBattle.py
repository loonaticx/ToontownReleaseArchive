...
# File: TownBattle.pyo (Python 2.0)
...
class TownBattle(StateData.StateData):
    ...
    def __init__(self, doneEvent):
        ...
        self.fsm = FSM.FSM('TownBattle', [
            ...
            State.State('SOS', self.enterSOS, self.exitSOS, [
                'Attack',
                'AttackWait'])], 'Off', 'Off')
        self.runPanelDoneEvent = 'run-panel-done'
        self.runPanel = TownBattleRunPanel.TownBattleRunPanel(self.runPanelDoneEvent)
    ...
    def enter(self, event, parentFSMState):
        ...
        toonbase.localToon.inventory.setActivateMode('battle', heal = 0)
    ...
    def adjustCogsAndToons(self, numCogs, luredIndices, trappedIndices, toons):
        ...
        if numCogs == self.numCogs and luredIndices == self.luredIndices and trappedIndices == self.trappedIndices and toonIds == self.toons:
            resetActivateMode = 0
        else:
            resetActivateMode = 1
        ...
        if resetActivateMode:
            ...
            toonbase.localToon.inventory.setActivateMode('battle', heal = canHeal, trap = canTrap, lure = canLure)
    ...
    def enterRun(self):
        self.runPanel.enter()
        self.accept(self.runPanelDoneEvent, self._TownBattle__handleRunPanelDone)
        return None

    
    def exitRun(self):
        self.ignore(self.runPanelDoneEvent)
        self.runPanel.exit()
        return None
